...
%
% Bearing 38 -> 52 degrees, bearing 142 -> -52 degrees. The two beams
...
\begin{tikzpicture}[scale=1]
  ...
  % North is drawn from O, not parked in a corner: it is the reference
  % direction the two bearings are swung from, so it has to sit at the
  % station. Same dashed style as the bearing figures later in the deck.
  \draw[north] (O) -- ($(O)+(0,1.9)$);
  \node[dim, above=1pt] at ($(O)+(0,1.9)$) {N};
...
\begin{tikzpicture}[scale=1]
  ...
  \node[figaccent, font=\small\bfseries] at (71:1.66) {$38^\circ$};
