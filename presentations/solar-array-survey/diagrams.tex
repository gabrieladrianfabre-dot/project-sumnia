...
\begin{tikzpicture}[scale=1]
  % roof slab + parapet
  \fill[solarmist] (0,0) rectangle (9.6,5.6);
  \draw[solarrule, line width=2pt] (0,0) rectangle (9.6,5.6);
  \draw[solarrule, line width=0.6pt] (0.18,0.18) rectangle (9.42,5.42);
  ...
  \foreach \x/\y/\w/\h in {6.4/3.7/1.5/1.2, 8.1/1.1/1.0/1.3, 6.6/1.2/0.9/0.7}{
    \fill[solarslate!25] (\x,\y) rectangle ++(\w,\h);
    \draw[solarslate!60, line width=0.6pt] (\x,\y) rectangle ++(\w,\h);
  }
  \draw[solarslate!45, line width=2.5pt] (7.15,3.7) -- (7.15,2.6) -- (8.6,2.6) -- (8.6,2.4);
  \node[dim, anchor=west] at (6.4,5.05) {existing HVAC plant};

  % north arrow
  \begin{scope}[shift={(0.85,4.35)}]
    \draw[solarslate, line width=0.9pt, -{Stealth[length=5pt]}] (0,0) -- (0,0.85);
  ...
  \fill[solaramber!30] (O) -- (A) -- (B) -- cycle;
  \draw[solaramber!80, line width=0.8pt] (A) -- (B);
  \draw[beam] (A) -- (O) -- (B);
  ...
  \node[solarnavy, font=\scriptsize\bfseries, align=center]
    at ($(O)!0.72!($(A)!0.5!(B)$)$) {unused\\corner};

  \node[dim, anchor=south west] at (0.25,-0.55) {Plan view · warehouse rooftop bounded by two structural beams};
\end{tikzpicture}}
...
\begin{tikzpicture}[scale=1]
  \foreach \cx/\mth/\lab/\name in {0/52/038/A, 4.4/-52/142/B}{
    \begin{scope}[shift={(\cx,0)}]
      \draw[solarrule, line width=0.8pt] (0,0) circle (1.45);
      \foreach \a/\t in {90/N, 0/E, -90/S, 180/W}{
        \draw[solarrule, line width=0.8pt] (\a:1.32) -- (\a:1.45);
        \node[dim] at (\a:1.72) {\t};
      }
      ...
      \node[solaramber, font=\small\bfseries] at ({(90+\mth)/2}:1.05) {$\lab^\circ$};
    \end{scope}}
  \node[dim, align=center, text width=8.4cm] at (2.2,-2.35)
    {A bearing is always measured \emph{clockwise from north} --- same origin, same sense, every time.};
\end{tikzpicture}}
...
\begin{tikzpicture}[scale=1]
  ...
  \fill[solaramber!22] (O) -- (52:1.05) arc[start angle=52, end angle=-52, radius=1.05] -- cycle;
...
\begin{tikzpicture}[scale=0.95]
  ...
  \begin{scope}
    \draw[solarrule, line width=0.8pt] (0,0) circle (1.5);
    \draw[north] (0,0) -- (90:1.5); \node[dim, above] at (90:1.5) {N};
    \fill[solaramber!22] (0,0) -- (52:0.85) arc[start angle=52, end angle=-52, radius=0.85] -- cycle;
    \draw[ray, -{Stealth[length=4pt]}] (0,0) -- (52:1.5);
    \draw[ray, -{Stealth[length=4pt]}] (0,0) -- (-52:1.5);
    \draw[angmark] (52:0.85) arc[start angle=52, end angle=-52, radius=0.85];
    \node[solaramber, font=\small\bfseries] at (0:1.12) {$104^\circ$};
    \node[dim] at (52:1.78) {$038^\circ$};
    \node[dim] at (-52:1.78) {$142^\circ$};
    \node[solarnavy, font=\small, align=center] at (0,-2.35)
  ...
  \begin{scope}[shift={(6.4,0)}]
    ...
    \fill[solaramber!22] (0,0) -- (50:0.85) arc[start angle=50, end angle=150, radius=0.85] -- cycle;
    ...
    \node[solaramber, font=\small\bfseries] at (100:1.15) {$100^\circ$};
...
\begin{tikzpicture}[scale=1]
  ...
  \fill[solaramber!20] (O) -- (A) -- (B) -- cycle;
  \draw[beam] (O) -- (A);
  \draw[beam] (O) -- (B);
  \draw[solarslate, line width=1pt, dashed] (A) -- (B);

  \draw[angmark] (52:0.8) arc[start angle=52, end angle=-52, radius=0.8];
  \node[solaramber, font=\bfseries] at (0:1.14) {$104^\circ$};
  ...
  \node[solarnavy, font=\small\bfseries, rotate=52,  anchor=south] at ($(O)!0.58!(A)$) {$b = 34.6$ m};
  \node[solarnavy, font=\small\bfseries, rotate=-52, anchor=north] at ($(O)!0.58!(B)$) {$a = 27.2$ m};
  \node[dim, anchor=west, align=left, text width=2.6cm] at ($(A)!0.5!(B)+(0.3,0)$)
    {side AB --- never measured, never needed};
\end{tikzpicture}}
...
\begin{tikzpicture}[scale=1]
  ...
  \draw[solarteal, line width=1.2pt, dashed] (A) -- (F);
  \draw[solarteal, line width=0.8pt] ($(F)+(0.22,0)$) -- ($(F)+(0.22,0.22)$) -- ($(F)+(0,0.22)$);
  \node[solarteal, font=\small\bfseries, rotate=90, anchor=south] at ($(A)!0.5!(F)$)
    {$h = b\sin C$};

  % angles at O
  \draw[angmark] (0:0.8) arc[start angle=0, end angle=104, radius=0.8];
  \node[solaramber, font=\small\bfseries] at (52:1.12) {$104^\circ$};
  \draw[solarteal, line width=1pt] (104:0.42) arc[start angle=104, end angle=180, radius=0.42];
  \node[solarteal, font=\scriptsize\bfseries] at (142:0.68) {$76^\circ$};
  ...
  \node[solarnavy, anchor=north west, font=\small, align=left] at (3.5,3.35)
    {Area $=\tfrac12 \times$ base $\times$ height\\[3pt]
     $\phantom{\text{Area}}=\tfrac12\, a\,(b\sin C)$};
  \node[solarslate, anchor=north west, font=\scriptsize, align=left] at (3.5,2.3)
...
\begin{tikzpicture}[scale=1]
  ...
  \end{scope}
  \draw[beam] (A) -- (O) -- (B);
  \draw[solarslate, line width=1pt] (A) -- (B);
  \foreach \p in {O,A,B}{\node[vertex] at (\p) {};}
  \node[vlabel, left=2pt]        at (O) {O};
  \node[vlabel, above right=1pt] at (A) {A};
  \node[vlabel, below right=1pt] at (B) {B};
  \node[dim, align=center, anchor=north] at (1.6,-3.0) {schematic only --- tilt, row spacing\\and shading are not modelled};
\end{tikzpicture}}
