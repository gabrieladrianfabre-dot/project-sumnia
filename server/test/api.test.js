...
let server, base, tmpDir, token

before(async () => {
  tmpDir = mkdtempSync(path.join(tmpdir(), 'sumnia-test-'))
  const store = new Store(path.join(tmpDir, 'items.json'))
  seedIfEmpty(store)
...
test('GET /api/items returns seed items and taxonomies', async () => {
...
test('filtering by branch and topic is two-axis AND', async () => {
...
test('GET single item and 404 for unknown id', async () => {
...
test('login rejects wrong password, accepts right one', async () => {
...
test('write endpoints reject missing/bad token', async () => {
...
test('create, update, delete item round-trip (with new taxonomy value)', async () => {
...
test('signup creates an account; duplicate emails and bad input are rejected', async () => {
...
test('user login works and rejects wrong credentials', async () => {
...
test('repo creation needs a signed-in user and records the creator', async () => {
...
test('community repo items: full CRUD without any token; vault stays locked', async () => {
  const repo = await (
...
test('GET /api/items?repo= scopes results to one repository', async () => {
  const repo = await (
...
test('create rejects missing required fields and bad year', async () => {
  const auth = { authorization: `Bearer ${token}` }
